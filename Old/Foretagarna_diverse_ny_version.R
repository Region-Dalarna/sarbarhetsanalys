...
diag_ranking <- function(region_vekt="20",
  ...
  # skapa fokusvariabel för att fokusera på valt län och riket
  nyforetagsamma_df_utskrift <- nyforetagsamma_df_utskrift %>%
    mutate(Kommun = Kommun %>% skapa_kortnamn_lan(),
           fokus = case_when(Kommun == "Dalarna" ~ 1,
                        Kommun == "Sverige" ~ 2,
                        TRUE ~ 0))
  ...
  if(diag_foretagsamma==TRUE){
    diagramtitel <- paste0("Andel företagsamma år ",max(foretagsamma_df_Dalarna$year))
