...
diag_ranking <- function(region_vekt="20",
  ...
  inlas_df<-inlas_df %>% 
    rename(Branschgrupp=AstSNI2007_namn.y) %>% 
      filter(AstKommun_namn==kommuner[[1]][i]) 
  ...
  inlas_utskrift<-rbind(inlas_df_lan_10_Pivot,inlas_kon_df,inlas_utb_df,inlas_bakgrund_df)
  
  inlas_utskrift[inlas_utskrift=="Andel_syss"]="Andel av förvärvsarbetande"
  inlas_utskrift[inlas_utskrift=="kvinna"]="Andel kvinnor"
  inlas_utskrift[inlas_utskrift=="Utrikes födda"]="Andel utrikes födda"
  inlas_utskrift[inlas_utskrift=="Högskoleutbildade"]="Andel högskoleutbildade"
  
  inlas_utskrift$Variabel <- factor(inlas_utskrift$Variabel, levels = c("Andel av förvärvsarbetande","Andel kvinnor","Andel utrikes födda","Andel högskoleutbildade")[4:1])
  ...
  if(diag_rank_andel_lan==TRUE){
    ...
    gg_obj <- SkapaStapelDiagram(skickad_df =inlas_df_kommun_10,
  ...
  if(diag_rank_andel_lan==TRUE){
    ...
    gg_obj <- SkapaStapelDiagram(skickad_df =inlas_utskrift,
                                 skickad_x_var = "Branschgrupp", 
                                 skickad_y_var = "andel", 
