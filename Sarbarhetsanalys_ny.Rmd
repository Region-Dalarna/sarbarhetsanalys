...
När det gäller nystartade företag har `r Nystartade_df %>% filter(year == max(year)) %>% filter(nystartade_ftg== max(nystartade_ftg)) %>% .$municipality` kommun flest, med ungefär `r round(Nystartade_df %>% filter(year == max(year)) %>% filter(nystartade_ftg== max(nystartade_ftg)) %>% .$nystartade_ftg,0)` nystartade företag per 1000 invånare `r max(Nystartade_df$year)`. Även Malung-Sälen och Falun hade fler nystartade företag (per 1000 invånare) än Sverige `r max(Nystartade_df$year)`. Glädjande är att antalet nystartade företag har ökat i de allra flesta av Dalarnas kommuner mellan 2011 och 2021, något som möjligtvis kan tillskrivas Coronapandemin. I Gagnef fördubblades exempelvis antalet nystartade företag mellan 2015 och 2021 (KOLLA UPP).
